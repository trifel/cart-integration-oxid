...
use Shopgate\Framework\ShopgateUnknownOxidConfigFields;
...
class ShopgateConfigOxid extends ShopgateConfig
{
    ...
    public function startup()
    {
        ...
        $this->unknownOxidConfigFields = new ShopgateUnknownOxidConfigFields(
            $this,
            ShopgateModule::getOxConfig(),
            ShopgateModule::getInstance()
        );
        ...
        $iUtfMode = ShopgateModule::getOxConfig()->getConfigParam('iUtfMode');
    ...
    {
        $oxConfig = ShopgateModule::getOxConfig();

        $result = array();
        foreach (ShopgateModule::getInstance()->_getConfig() as $sConfigKey => $aOptions) {
            $sValue = $oxConfig->getConfigParam(ShopgateModule::getInstance()->getOxidConfigKey($sConfigKey));
            if ($sValue !== null) {
                $result[$sConfigKey] = $sValue;
            }
        }

        $shopDir            = ShopgateModule::getOxConfig()->getConfigParam('sShopDir');
        ...
        if (version_compare(ShopgateModule::getOxConfig()->getVersion(), '4.2.0', '<')) {
        ...
        $mobileHeaderParent  = $oxConfig->getConfigParam(
            ShopgateModule::getInstance()->getOxidConfigKey('mobile_header_parent')
        );
        $mobileHeaderPrepend = $oxConfig->getConfigParam(
            ShopgateModule::getInstance()->getOxidConfigKey('mobile_header_prepend')
        );
    ...
    {
        $oxConfig     = ShopgateModule::getOxConfig();
        $configFields = ShopgateModule::getInstance()->_getConfig();
        ...
        foreach ($fieldList as $field) {
            ...
            $oxidName = ShopgateModule::getInstance()->getOxidConfigKey($field);
    ...
    public function isVariantParentBuyable()
    {
        if ($this->variant_parent_buyable == 'oxid') {
            return ShopgateModule::getOxConfig()->getConfigParam('blVariantParentBuyable');
        }
